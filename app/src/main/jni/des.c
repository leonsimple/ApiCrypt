...
unsigned char key[] = { 0x70, 0x69, 0x6e, 0x67, 0x61, 0x6e, 0x31, 0x32,0x00 };
/// DES 中密钥的转换表
...
void des_encipher(const unsigned char *plaintext, unsigned char **ciphertext) {
    size_t nPlaintextLen = strlen(plaintext);//明文长度
    size_t nCiphertextLen =  nPlaintextLen ;

    size_t remainders = nPlaintextLen % 8;
    if (remainders)//需要填充
    {
        nCiphertextLen = nCiphertextLen + (8 -  remainders);//密文长度，加了填充位
    }

    unsigned char *buf = malloc(nCiphertextLen);
    memset(buf, 0, nCiphertextLen);

    for (int i = 0; plaintext[i]; i += 8) {
        des_main( plaintext + i, buf + i, key, encipher);
    }

    //BASE64编码
    size_t mBase64Len = (nCiphertextLen * 4) / 3  + 4;
    *ciphertext = (unsigned char *)malloc(mBase64Len);
    memset(*ciphertext, 0, mBase64Len);
    
    base64_encode(buf, *ciphertext, nCiphertextLen, 0);
...
void des_decipher(const unsigned char *ciphertext, unsigned char **plaintext) {
    //BASE64解码
    size_t mBase64Len = 3 * strlen(ciphertext) / 4 + 1;
    unsigned char *pBase64Buffer = (unsigned char *)malloc(mBase64Len);
    memset(pBase64Buffer, 0, mBase64Len);
    size_t nCiphertextLen = base64_decode(ciphertext, pBase64Buffer, strlen(ciphertext));

    *plaintext = (unsigned char *)malloc(nCiphertextLen + 1);
    for (int i = 0; pBase64Buffer[i]; i += 8) {
        des_main(pBase64Buffer + i, *plaintext + i, key, decipher);
    }
    return;
}
